...
async function buildConfigAndIndexingFunctions({
  ...
  const logs = [];
  let databaseConfig;
  const sqliteDir = config.database?.kind === "sqlite" && config.database.directory ? path.resolve(config.database.directory) : path.join(ponderDir, "sqlite");
  ...
  const buildContext = async () => {
    ...
    return {
      ...
      session: {
        ...
        total_memory_bytes: os2.totalmem()
      }
    };
  };
  let context = void 0;
...
function Table(props) {
  ...
  return /* @__PURE__ */ React2.createElement(Box, { flexDirection: "column" }, /* @__PURE__ */ React2.createElement(Box, { flexDirection: "row", key: "title" }, columns.map(({ title, align }, index) => /* @__PURE__ */ React2.createElement(React2.Fragment, { key: `title-${title}` }, /* @__PURE__ */ React2.createElement(Text2, null, "\u2502"), /* @__PURE__ */ React2.createElement(
    Box,
    {
  ...
  ))), /* @__PURE__ */ React2.createElement(Text2, null, "\u2502")), /* @__PURE__ */ React2.createElement(Box, { flexDirection: "row", key: "border" }, /* @__PURE__ */ React2.createElement(Text2, null, "\u251C"), columnWidths.map((width, index) => (
      ...
      Table_default,
      {
        ...
        columns: [
          ...
          },
          {
            ...
            key: "averageDuration",
            align: "right",
            format: (v) => v > 0 ? v < 1 ? `${(v * 1e3).toFixed(2)}\u03BCs` : `${v.toFixed(2)}ms` : "-"
          }
        ]
      }
    ), /* @__PURE__ */ React3.createElement(Text3, null, " "));
...
function createReadonlyPool(config) {
  return new pg.Pool({
    // https://stackoverflow.com/questions/59155572/how-to-set-query-timeout-in-relation-to-statement-timeout
    statement_timeout: 2 * 60 * 1e3,
    // 2 minutes
    ...
    return this.db.wrap({ method: "setup" }, async () => {
      const attemptSetup = async () => {
        return await this.db.transaction().execute(async (tx) => {
          ...
          await tx.schema.withSchema(this.userNamespace).createTable("_ponder_meta").addColumn("key", "text", (col) => col.primaryKey()).addColumn("value", "jsonb").ifNotExists().execute();
          await tx.withSchema(this.userNamespace).insertInto("_ponder_meta").values({ key: "status", value: null }).onConflict((oc) => oc.column("key").doUpdateSet({ value: null })).execute();
          ...
          if (this.common.options.command === "start" && previousLockRow.build_id === this.buildId && previousLockRow.finalized_checkpoint !== encodeCheckpoint(zeroCheckpoint)) {
            this.common.logger.info({
              service: "database",
              msg: `Detected cache hit for build '${this.buildId}' in schema '${this.userNamespace}' last active ${formatEta(Date.now() - previousLockRow.heartbeat_at)} ago`
            });
            for (const [tableName, table] of Object.entries(
              getTables(schema)
            )) {
    ...
    await this.db.wrap({ method: "publish" }, async () => {
      ...
      await this.db.transaction().execute(async (tx) => {
        ...
        for (const tableName of Object.keys(getTables(this.schema)).concat(
          ...
          if (isView) {
            await tx.schema.withSchema(publishSchema).dropView(tableName).ifExists().cascade().execute();
            this.common.logger.debug({
              service: "database",
...
function createReadonlySqliteDatabase(file, options) {
  ensureDirExists(file);
      ...
      const attemptSetup = async () => {
        return await this.db.transaction().execute(async (tx) => {
          ...
          if (this.common.options.command === "start" && previousLockRow.build_id === this.buildId && previousLockRow.finalized_checkpoint !== encodeCheckpoint(zeroCheckpoint)) {
            this.common.logger.info({
              service: "database",
              msg: `Detected cache hit for build '${this.buildId}' in database file '${this.userNamespace}.db' last active ${formatEta(Date.now() - previousLockRow.heartbeat_at)} ago`
            });
            for (const [tableName, table] of Object.entries(
              getTables(schema)
            )) {
      ...
      Object.entries(getTables(schema)).flatMap(([tableName, table]) => {
        ...
        return Object.entries(table.constraints).map(async ([name, index]) => {
          await this.db.wrap({ method: "createIndexes" }, async () => {
            ...
            await this.db.executeQuery(
              sql4`CREATE INDEX ${sql4.ref(this.userNamespace)}.${sql4.ref(indexName)} ON ${sql4.table(
                tableName
              )} (${sql4.raw(columns)})`.compile(this.db)
            );
          });
          this.common.logger.info({
            service: "database",
...
function encodeRecord({
  ...
  if (skipValidation === false)
    validateRecord({ record, table, schema });
  ...
  switch (column[" type"]) {
      ...
      if (isListColumn(column)) {
        ...
        if (getEnums(schema)[column[" enum"]].includes(value) === false) {
          throw new CheckConstraintError(
            `Unable to encode ${value} as a '${column[" enum"]}' enum. Got '${value}' but expected one of [${getEnums(schema)[column[" enum"]].join(", ")}].`
          );
        }
      }
      break;
    ...
    case "scalar": {
      if (value === void 0 || value === null) {
        if (isOptionalColumn(column))
          break;
        const error = new NotNullConstraintError(
          `Unable to encode ${value} as a ${column[" scalar"]}. Got '${typeof value}' but expected type '${scalarToTsType[column[" scalar"]]}'.`
        );
        error.meta.push(
      ...
      switch (column[" scalar"]) {
        ...
        case "float": {
          if (typeof value !== "number") {
            throw new StoreError(
              `Unable to encode ${value} as an ${column[" scalar"]}. Got type '${typeof value}' but expected type 'number'.`
            );
          }
          break;
        }
...
var getHistoricalStore = ({
  ...
  const normalizeHex = (hex) => padHex(hex, {
    size: Math.ceil((hex.length - 2) / 2),
    dir: "left"
  }).toLowerCase();
  const getCacheKey = (id, tableName) => {
    if (tables[tableName].table.id[" scalar"] === "hex")
      return normalizeHex(id);
    if (typeof id === "bigint")
      return `#Bigint.${id}`;
    return id;
  };
  const normalizeRecord = (record, tableName) => {
    for (const [columnName, column] of Object.entries(
      tables[tableName].table
    )) {
      if (isMaterialColumn(column) && record[columnName] === void 0) {
        record[columnName] = null;
      }
      if ((isScalarColumn(column) || isReferenceColumn(column)) && column[" scalar"] === "hex" && typeof record[columnName] === "string") {
        record[columnName] = normalizeHex(record[columnName]);
      }
    }
  };
    ...
    worker: async ({ isFullFlush }) => {
      ...
      await Promise.all(
        Object.entries(storeCache).map(async ([tableName, tableStoreCache]) => {
          ...
          let insertRecords;
          if (isFullFlush) {
            insertRecords = cacheEntries.filter(({ type }) => type === "insert").map(({ record }) => record);
          } else {
            insertRecords = cacheEntries.filter(
              ({ type, opIndex }) => type === "insert" && opIndex < flushIndex
            ).map(({ record }) => record);
          }
          if (insertRecords.length !== 0) {
            ...
            for (let i = 0, len = insertRecords.length; i < len; i += batchSize) {
              await db.wrap({ method: `${tableName}.flush` }, async () => {
                const _insertRecords = insertRecords.slice(i, i + batchSize).map(
                  (record) => encodeRecord({
                    record,
                    table,
                    schema,
                ...
                await db.withSchema(namespaceInfo.userNamespace).insertInto(tableName).values(_insertRecords).execute().catch((_error) => {
                  const error = _error;
                  common.logger.error({
                    service: "indexing",
          ...
          let updateRecords;
          if (isFullFlush) {
            updateRecords = cacheEntries.filter(({ type }) => type === "update").map(({ record }) => record);
          } else {
            updateRecords = cacheEntries.filter(
              ({ type, opIndex }) => type === "update" && opIndex < flushIndex
            ).map(({ record }) => record);
          }
          if (updateRecords.length !== 0) {
            ...
            for (let i = 0, len = updateRecords.length; i < len; i += batchSize) {
              await db.wrap({ method: `${tableName}.flush` }, async () => {
                const _updateRecords = updateRecords.slice(i, i + batchSize).map(
                  (record) => encodeRecord({
                    record,
                    table,
                    schema,
                ...
                await db.withSchema(namespaceInfo.userNamespace).insertInto(tableName).values(_updateRecords).onConflict(
                  (oc) => oc.column("id").doUpdateSet(
                    (eb) => Object.entries(table).reduce(
                      (acc, [colName, column]) => {
                        if (colName !== "id") {
                          if (isMaterialColumn(column)) {
  ...
  const _findUnique = async ({
    ...
    const record = await db.withSchema(namespaceInfo.userNamespace).selectFrom(tableName).selectAll().where("id", "=", encodedId).executeTakeFirst();
    if (record === void 0)
      return null;
  ...
  return {
    ...
    update: async ({
      ...
      return db.wrap({ method: `${tableName}.findUnique` }, async () => {
        ...
        if (cacheEntry === void 0) {
          const record2 = isCacheExhaustive ? null : await _findUnique({ tableName, id });
          if (record2 === null) {
            throw new RecordNotFoundError(
              "No existing record was found with the specified ID"
            );
          }
          cacheEntry = { type: "update", opIndex: 0, bytes: 0, record: record2 };
          ...
          if (cacheEntry.type === "find") {
            cacheEntry.type = "update";
          }
        }
        const update = typeof data === "function" ? data({ current: structuredClone(cacheEntry.record) }) : data;
    ...
    updateMany: async ({
      ...
      if (typeof data === "function") {
        const query2 = db.withSchema(namespaceInfo.userNamespace).selectFrom(tableName).selectAll().where((eb) => buildWhereConditions({ eb, where, table, encoding })).orderBy("id", "asc");
        const records = [];
        let cursor = null;
        while (true) {
          const _records = await db.wrap(
            ...
            async () => {
              const latestRecords = await query2.limit(common.options.databaseMaxRowLimit).$if(cursor !== null, (qb) => qb.where("id", ">", cursor)).execute();
              const records2 = [];
              for (const latestRecord of latestRecords) {
                ...
                const record = await db.withSchema(namespaceInfo.userNamespace).updateTable(tableName).set(updateRecord).where("id", "=", latestRecord.id).returningAll().executeTakeFirstOrThrow().catch((err) => {
                  throw parseStoreError(err, updateObject);
                });
                records2.push(record);
              }
              return records2.map(
                (record) => decodeRecord({ record, table, encoding })
              );
            }
          );
        ...
        return db.wrap({ method: `${tableName}.updateMany` }, async () => {
          ...
          const records = await db.with(
            "latestRows(id)",
            (db2) => db2.withSchema(namespaceInfo.userNamespace).selectFrom(tableName).select("id").where(
              (eb) => buildWhereConditions({ eb, where, table, encoding })
            )
          ).withSchema(namespaceInfo.userNamespace).updateTable(tableName).set(updateRecord).from("latestRows").where(`${tableName}.id`, "=", sql5.ref("latestRows.id")).returningAll().execute().catch((err) => {
    ...
    upsert: async ({
      ...
      return db.wrap({ method: `${tableName}.upsert` }, async () => {
        ...
        if (cacheEntry?.record) {
          const _update = typeof update === "function" ? update({ current: structuredClone(cacheEntry.record) }) : update;
          const record = cacheEntry.record;
          ...
          const record = structuredClone(create5);
          record.id = id;
          normalizeRecord(record, tableName);
          validateRecord({ record, table: tables[tableName].table, schema });
          const bytes = getBytesSize(record);
    ...
    delete: async ({
      ...
      return db.wrap({ method: `${tableName}.delete` }, async () => {
        ...
        if (isCacheExhaustive || cacheEntry?.record === null) {
          ...
          const table = schema[tableName].table;
          const deletedRecord = await db.withSchema(namespaceInfo.userNamespace).deleteFrom(tableName).where(
            "id",
            "=",
            encodeValue({ value: id, column: table.id, encoding })
          ).returning(["id"]).executeTakeFirst().catch((err) => {
...
var getMetadataStore = ({
  ...
  getStatus: async () => {
    return db.wrap({ method: "_ponder_meta.getStatus()" }, async () => {
      const metadata = await db.withSchema(namespaceInfo.userNamespace).selectFrom("_ponder_meta").select("value").where("key", "=", "status").executeTakeFirst();
      if (metadata.value === null)
        return null;
      return encoding === "sqlite" ? JSON.parse(metadata.value) : metadata.value;
    });
  },
  setStatus: (status) => {
    return db.wrap({ method: "_ponder_meta.setStatus()" }, async () => {
      await db.withSchema(namespaceInfo.userNamespace).insertInto("_ponder_meta").values({
        key: "status",
    ...
    return db.wrap({ method: `${tableName}.findMany` }, async () => {
      ...
      for (const [column, direction] of orderByConditions) {
        query2 = query2.orderBy(
          column,
          encoding === "sqlite" ? direction : direction === "asc" ? sql6`asc nulls first` : sql6`desc nulls last`
        );
      }
      ...
      if (after === null && before === null) {
        ...
        const records = await query2.execute().then(
          (records2) => records2.map((record) => decodeRecord({ record, table, encoding }))
        );
      ...
      if (after !== null) {
        ...
        const records = await query2.execute().then(
          (records2) => records2.map((record) => decodeRecord({ record, table, encoding })).reverse()
        );
        if (records.length === 0) {
          return {
            items: records,
...
var getRealtimeStore = ({
  ...
  create: ({
    ...
    return db.wrap({ method: `${tableName}.create` }, async () => {
      ...
      return await db.transaction().execute(async (tx) => {
        const record = await tx.withSchema(namespaceInfo.userNamespace).insertInto(tableName).values(createRecord).returningAll().executeTakeFirstOrThrow().catch((err) => {
    ...
    return db.wrap({ method: `${tableName}.createMany` }, async () => {
      ...
      await db.transaction().execute(async (tx) => {
        ...
        for (let i = 0, len = data.length; i < len; i += batchSize) {
          const createRecords = data.slice(i, i + batchSize).map(
            (d) => encodeRecord({
              record: d,
              table,
              encoding,
          ...
          const _records = await tx.withSchema(namespaceInfo.userNamespace).insertInto(tableName).values(createRecords).returningAll().execute().catch((err) => {
  ...
  update: ({
    ...
    return db.wrap({ method: `${tableName}.update` }, async () => {
      ...
      const record = await db.transaction().execute(async (tx) => {
        const latestRecord = await tx.withSchema(namespaceInfo.userNamespace).selectFrom(tableName).selectAll().where("id", "=", encodedId).executeTakeFirstOrThrow().catch((err) => {
          throw parseStoreError(err, { id, data: "(function)" });
        });
        const updateObject = typeof data === "function" ? data({
        ...
        const updateResult = await tx.withSchema(namespaceInfo.userNamespace).updateTable(tableName).set(updateRecord).where("id", "=", encodedId).returningAll().executeTakeFirstOrThrow().catch((err) => {
    ...
    while (true) {
      const _records = await db.wrap(
        ...
        () => db.transaction().execute(async (tx) => {
          const latestRecords = await tx.withSchema(namespaceInfo.userNamespace).selectFrom(tableName).selectAll().where(
          ...
          ).orderBy("id", "asc").limit(common.options.databaseMaxRowLimit).$if(cursor !== null, (qb) => qb.where("id", ">", cursor)).execute();
          const records2 = [];
          for (const latestRecord of latestRecords) {
            const updateObject = typeof data === "function" ? data({
              current: decodeRecord({
            ...
            const record = await tx.withSchema(namespaceInfo.userNamespace).updateTable(tableName).set(updateRecord).where("id", "=", latestRecord.id).returningAll().executeTakeFirstOrThrow().catch((err) => {
              throw parseStoreError(err, updateObject);
            });
            records2.push(record);
            await tx.withSchema(namespaceInfo.internalNamespace).insertInto(namespaceInfo.internalTableIds[tableName]).values({
  ...
  upsert: ({
    ...
    return db.wrap({ method: `${tableName}.upsert` }, async () => {
      ...
      const record = await db.transaction().execute(async (tx) => {
        const latestRecord = await tx.withSchema(namespaceInfo.userNamespace).selectFrom(tableName).selectAll().where("id", "=", encodedId).executeTakeFirst();
        if (latestRecord === void 0) {
          const record3 = await tx.withSchema(namespaceInfo.userNamespace).insertInto(tableName).values(createRecord).returningAll().executeTakeFirstOrThrow().catch((err) => {
        ...
        const updateObject = typeof update === "function" ? update({
          current: decodeRecord({
            record: latestRecord,
            table,
            encoding
        ...
        const record2 = await tx.withSchema(namespaceInfo.userNamespace).updateTable(tableName).set(updateRecord).where("id", "=", encodedId).returningAll().executeTakeFirstOrThrow().catch((err) => {
          const prettyObject = { id };
          for (const [key, value] of Object.entries(create5))
            prettyObject[`create.${key}`] = value;
  ...
  delete: ({
    ...
    return db.wrap({ method: `${tableName}.delete` }, async () => {
      ...
      const isDeleted = await db.transaction().execute(async (tx) => {
        const record = await tx.withSchema(namespaceInfo.userNamespace).selectFrom(tableName).selectAll().where("id", "=", encodedId).executeTakeFirst();
        const deletedRecord = await tx.withSchema(namespaceInfo.userNamespace).deleteFrom(tableName).where("id", "=", encodedId).returning(["id"]).executeTakeFirst().catch((err) => {
          throw parseStoreError(err, { id });
        });
        if (record !== void 0) {
          await tx.withSchema(namespaceInfo.internalNamespace).insertInto(namespaceInfo.internalTableIds[tableName]).values({
...
import { checksumAddress as checksumAddress2, hexToBigInt as hexToBigInt2, hexToNumber as hexToNumber2 } from "viem";
...
var PostgresSyncStore = class {
    ...
    return this.db.wrap({ method: "getLogFilterIntervals" }, async () => {
      ...
      for (const fragment of fragments) {
        await this.db.transaction().execute(async (tx) => {
          while (true) {
            ...
            const existingIntervals = await tx.deleteFrom("logFilterIntervals").where(
              "id",
              "in",
              tx.selectFrom("logFilterIntervals").where("logFilterId", "=", logFilterId).select("id").limit(this.common.options.syncStoreMaxIntervals)
            ...
            if (mergedIntervalRows.length === this.common.options.syncStoreMaxIntervals) {
              throw new NonRetryableError(
                `'logFilterIntervals' table for chain '${chainId}' has reached an unrecoverable level of fragmentation.`
              );
            }
            if (existingIntervals.length !== this.common.options.syncStoreMaxIntervals)
              break;
          }
        });
      }
      const intervals = await this.db.with(
        ...
        () => sql7`( values ${sql7.join(
          fragments.map(
            (f) => sql7`( ${sql7.val(f.id)}, ${sql7.val(f.address)}, ${sql7.val(f.topic0)}, ${sql7.val(
              f.topic1
            )}, ${sql7.val(f.topic2)}, ${sql7.val(f.topic3)}, ${sql7.lit(
        ...
        for (const fragment of fragments) {
          await this.db.transaction().execute(async (tx) => {
            while (true) {
              ...
              const existingIntervals = await tx.deleteFrom("factoryLogFilterIntervals").where(
                "id",
                "in",
                tx.selectFrom("factoryLogFilterIntervals").where("factoryId", "=", factoryId).select("id").limit(this.common.options.syncStoreMaxIntervals)
              ).returning(["startBlock", "endBlock"]).execute();
              const mergedIntervals = intervalUnion(
                existingIntervals.map((i) => [
                  Number(i.startBlock),
          ...
          () => sql7`( values ${sql7.join(
            fragments.map(
              (f) => sql7`( ${sql7.val(f.id)}, ${sql7.val(f.address)}, ${sql7.val(
                f.eventSelector
              )}, ${sql7.val(f.childAddressLocation)}, ${sql7.val(f.topic0)}, ${sql7.val(
                f.topic1
              )}, ${sql7.val(f.topic2)}, ${sql7.val(f.topic3)}, ${sql7.lit(
                f.includeTransactionReceipts
              )} )`
  ...
  getTraceFilterIntervals = async ({
    ...
    return this.db.wrap({ method: "getTraceFilterIntervals" }, async () => {
      ...
      for (const fragment of fragments) {
        await this.db.transaction().execute(async (tx) => {
          while (true) {
            ...
            const existingIntervals = await tx.deleteFrom("traceFilterIntervals").where(
              "id",
              "in",
              tx.selectFrom("traceFilterIntervals").where("traceFilterId", "=", traceFilterId).select("id").limit(this.common.options.syncStoreMaxIntervals)
            ).returning(["startBlock", "endBlock"]).execute();
            const mergedIntervals = intervalUnion(
              existingIntervals.map((i) => [
                Number(i.startBlock),
      ...
      const intervals = await this.db.with(
        ...
        () => sql7`( values ${sql7.join(
          fragments.map(
            (f) => sql7`( ${sql7.val(f.id)}, ${sql7.val(f.fromAddress)}, ${sql7.val(f.toAddress)} )`
          )
        )} )`
        ...
        for (const fragment of fragments) {
          await this.db.transaction().execute(async (tx) => {
            while (true) {
              ...
              const existingIntervals = await tx.deleteFrom("factoryTraceFilterIntervals").where(
                "id",
                "in",
                tx.selectFrom("factoryTraceFilterIntervals").where("factoryId", "=", factoryId).select("id").limit(this.common.options.syncStoreMaxIntervals)
              ).returning(["startBlock", "endBlock"]).execute();
              const mergedIntervals = intervalUnion(
                existingIntervals.map((i) => [
                  Number(i.startBlock),
        ...
        const intervals = await this.db.with(
          ...
          () => sql7`( values ${sql7.join(
            fragments.map(
              (f) => sql7`( ${sql7.val(f.id)}, ${sql7.val(f.address)}, ${sql7.val(
                f.eventSelector
              )}, ${sql7.val(f.childAddressLocation)}, ${sql7.val(f.fromAddress)} )`
            )
          )} )`
  ...
  createLogCheckpoint = (log, block, chainId) => {
    return encodeCheckpoint({
      blockTimestamp: Number(BigInt(block.timestamp)),
      chainId: BigInt(chainId),
      blockNumber: hexToBigInt2(block.number),
      transactionIndex: hexToBigInt2(log.transactionIndex),
      eventType: EVENT_TYPES.logs,
      eventIndex: hexToBigInt2(log.logIndex)
    });
  };
  ...
  async *getEvents({
    ...
    while (true) {
      ...
      const events = await this.db.wrap({ method: "getEvents" }, async () => {
              ...
              for (const blockSource of blockSources) {
                exprs.push(
                  eb.and([
                    ...
                    sql7`(number - ${sql7.val(blockSource.criteria.offset)}) % ${sql7.val(
        ...
        ).where("events.checkpoint", ">", fromCursor).where("events.checkpoint", "<=", toCursor).orderBy("events.checkpoint", "asc").limit(this.common.options.syncEventsQuerySize).execute();
      ...
      if (events.length === 0) {
        ...
        this.seconds = Math.round(
          Math.min(
            this.seconds / events.length * this.common.options.syncEventsQuerySize * 0.9,
            this.seconds * 2
          )
        );
        fromCursor = toCursor;
      ...
      if (events.length > 0)
        yield events;
      if (events.length !== this.common.options.syncEventsQuerySize && toCursor === maxToCursor) {
        break;
      }
    }
  }
  ...
  async pruneByChainId({ chainId, block }) {
    await this.db.wrap(
      { method: "pruneByChainId" },
      () => this.db.transaction().execute(async (tx) => {
        await tx.with(
          "deleteLogFilter(logFilterId)",
          (qb) => qb.selectFrom("logFilterIntervals").innerJoin("logFilters", "logFilterId", "logFilters.id").select("logFilterId").where("chainId", "=", chainId).where("startBlock", ">=", BigInt(block))
        ).deleteFrom("logFilterIntervals").where(
          "logFilterId",
          "in",
          sql7`(SELECT "logFilterId" FROM ${sql7.table("deleteLogFilter")})`
        ).execute();
        await tx.with(
          "updateLogFilter(logFilterId)",
          (qb) => qb.selectFrom("logFilterIntervals").innerJoin("logFilters", "logFilterId", "logFilters.id").select("logFilterId").where("chainId", "=", chainId).where("startBlock", "<", BigInt(block)).where("endBlock", ">", BigInt(block))
        ).updateTable("logFilterIntervals").set({
          endBlock: BigInt(block)
        }).where(
          "logFilterId",
          "in",
          sql7`(SELECT "logFilterId" FROM ${sql7.table("updateLogFilter")})`
        ).execute();
        await tx.with(
          ...
          ).select("factoryId").where("chainId", "=", chainId).where("startBlock", ">=", BigInt(block))
        ).deleteFrom("factoryLogFilterIntervals").where(
          "factoryId",
          "in",
          sql7`(SELECT "factoryId" FROM ${sql7.table("deleteFactoryLogFilter")})`
        ).execute();
        await tx.with(
          ...
          (qb) => qb.selectFrom("factoryLogFilterIntervals").innerJoin(
            ...
            "factoryId",
            "factoryLogFilters.id"
          ).select("factoryId").where("chainId", "=", chainId).where("startBlock", "<", BigInt(block)).where("endBlock", ">", BigInt(block))
        ).updateTable("factoryLogFilterIntervals").set({
          endBlock: BigInt(block)
        }).where(
          "factoryId",
          "in",
          sql7`(SELECT "factoryId" FROM ${sql7.table("updateFactoryLogFilter")})`
        ).execute();
        await tx.with(
          "deleteTraceFilter(traceFilterId)",
          (qb) => qb.selectFrom("traceFilterIntervals").innerJoin("traceFilters", "traceFilterId", "traceFilters.id").select("traceFilterId").where("chainId", "=", chainId).where("startBlock", ">=", BigInt(block))
        ).deleteFrom("traceFilterIntervals").where(
          "traceFilterId",
          "in",
          sql7`(SELECT "traceFilterId" FROM ${sql7.table("deleteTraceFilter")})`
        ).execute();
        await tx.with(
          "updateTraceFilter(traceFilterId)",
          (qb) => qb.selectFrom("traceFilterIntervals").innerJoin("traceFilters", "traceFilterId", "traceFilters.id").select("traceFilterId").where("chainId", "=", chainId).where("startBlock", "<", BigInt(block)).where("endBlock", ">", BigInt(block))
        ).updateTable("traceFilterIntervals").set({
          endBlock: BigInt(block)
        }).where(
          "traceFilterId",
          "in",
          sql7`(SELECT "traceFilterId" FROM ${sql7.table("updateTraceFilter")})`
        ).execute();
        await tx.with(
          ...
          "factoryId",
          "in",
          sql7`(SELECT "factoryId" FROM ${sql7.table("deleteFactoryTraceFilter")})`
        ).execute();
        await tx.with(
          ...
          "factoryId",
          "in",
          sql7`(SELECT "factoryId" FROM ${sql7.table("updateFactoryTraceFilter")})`
        ).execute();
        await tx.with(
          "deleteBlockFilter(blockFilterId)",
          (qb) => qb.selectFrom("blockFilterIntervals").innerJoin("blockFilters", "blockFilterId", "blockFilters.id").select("blockFilterId").where("chainId", "=", chainId).where("startBlock", ">=", BigInt(block))
        ).deleteFrom("blockFilterIntervals").where(
          "blockFilterId",
          "in",
          sql7`(SELECT "blockFilterId" FROM ${sql7.table("deleteBlockFilter")})`
        ).execute();
        await tx.with(
          "updateBlockFilter(blockFilterId)",
          (qb) => qb.selectFrom("blockFilterIntervals").innerJoin("blockFilters", "blockFilterId", "blockFilters.id").select("blockFilterId").where("chainId", "=", chainId).where("startBlock", "<", BigInt(block)).where("endBlock", ">", BigInt(block))
        ).updateTable("blockFilterIntervals").set({
          endBlock: BigInt(block)
        }).where(
          "blockFilterId",
          "in",
          sql7`(SELECT "blockFilterId" FROM ${sql7.table("updateBlockFilter")})`
        ).execute();
        await tx.deleteFrom("logs").where("chainId", "=", chainId).where("blockNumber", ">=", BigInt(block)).execute();
        await tx.deleteFrom("blocks").where("chainId", "=", chainId).where("number", ">=", BigInt(block)).execute();
        await tx.deleteFrom("rpcRequestResults").where("chainId", "=", chainId).where("blockNumber", ">=", BigInt(block)).execute();
        await tx.deleteFrom("callTraces").where("chainId", "=", chainId).where("blockNumber", ">=", BigInt(block)).execute();
        await tx.deleteFrom("transactions").where("chainId", "=", chainId).where("blockNumber", ">=", BigInt(block)).execute();
        await tx.deleteFrom("transactionReceipts").where("chainId", "=", chainId).where("blockNumber", ">=", BigInt(block)).execute();
      })
    );
  }
};
    ...
    return this.db.wrap({ method: "getLogFilterIntervals" }, async () => {
      ...
      const intervals = await this.db.with(
        ...
        () => sql8`( values ${sql8.join(
          fragments.map(
            (f) => sql8`( ${sql8.val(f.id)}, ${sql8.val(f.address)}, ${sql8.val(f.topic0)}, ${sql8.val(
              f.topic1
            )}, ${sql8.val(f.topic2)}, ${sql8.val(f.topic3)}, ${sql8.lit(
              f.includeTransactionReceipts
            )} )`
          )
        )} )`
          ...
          () => sql8`( values ${sql8.join(
            fragments.map(
              (f) => sql8`( ${sql8.val(f.id)}, ${sql8.val(f.address)}, ${sql8.val(
                f.eventSelector
              )}, ${sql8.val(f.childAddressLocation)}, ${sql8.val(f.topic0)}, ${sql8.val(
                f.topic1
              )}, ${sql8.val(f.topic2)}, ${sql8.val(f.topic3)}, ${sql8.lit(
                f.includeTransactionReceipts
              )} )`
  ...
  getTraceFilterIntervals = async ({
    ...
    return this.db.wrap({ method: "getTraceFilterIntervals" }, async () => {
      ...
      const intervals = await this.db.with(
        ...
        () => sql8`( values ${sql8.join(
          fragments.map(
            (f) => sql8`( ${sql8.val(f.id)}, ${sql8.val(f.fromAddress)}, ${sql8.val(f.toAddress)} )`
          )
        )} )`
      ).selectFrom("traceFilterIntervals").innerJoin("traceFilters", "traceFilterId", "traceFilters.id").innerJoin("traceFilterFragments", (join) => {
        return join.on(
        ...
        const intervals = await this.db.with(
          ...
          () => sql8`( values ${sql8.join(
            fragments.map(
              (f) => sql8`( ${sql8.val(f.id)}, ${sql8.val(f.address)}, ${sql8.val(
                f.eventSelector
              )}, ${sql8.val(f.childAddressLocation)}, ${sql8.val(f.fromAddress)} )`
            )
          )} )`
        ).selectFrom("factoryTraceFilterIntervals").innerJoin(
          "factoryTraceFilters",
  ...
  async pruneByChainId({ chainId, block }) {
    await this.db.wrap(
      ...
      () => this.db.transaction().execute(async (tx) => {
        await tx.with(
          "deleteLogFilter(logFilterId)",
          (qb) => qb.selectFrom("logFilterIntervals").innerJoin("logFilters", "logFilterId", "logFilters.id").select("logFilterId").where("chainId", "=", chainId).where("startBlock", ">=", encodeAsText(block))
        ).deleteFrom("logFilterIntervals").where(
          "logFilterId",
          "in",
          sql8`(SELECT "logFilterId" FROM ${sql8.table("deleteLogFilter")})`
        ).execute();
        await tx.with(
          "updateLogFilter(logFilterId)",
          (qb) => qb.selectFrom("logFilterIntervals").innerJoin("logFilters", "logFilterId", "logFilters.id").select("logFilterId").where("chainId", "=", chainId).where("startBlock", "<", encodeAsText(block)).where("endBlock", ">", encodeAsText(block))
        ).updateTable("logFilterIntervals").set({
          endBlock: encodeAsText(block)
        }).where(
          "logFilterId",
          "in",
          sql8`(SELECT "logFilterId" FROM ${sql8.table("updateLogFilter")})`
        ).execute();
        await tx.with(
          "deleteFactoryLogFilter(factoryId)",
          (qb) => qb.selectFrom("factoryLogFilterIntervals").innerJoin(
            ...
            "factoryId",
            "factoryLogFilters.id"
          ).select("factoryId").where("chainId", "=", chainId).where("startBlock", ">=", encodeAsText(block))
        ).deleteFrom("factoryLogFilterIntervals").where(
          "factoryId",
          "in",
          sql8`(SELECT "factoryId" FROM ${sql8.table("deleteFactoryLogFilter")})`
        ).execute();
        await tx.with(
          "updateFactoryLogFilter(factoryId)",
          (qb) => qb.selectFrom("factoryLogFilterIntervals").innerJoin(
            ...
            "factoryId",
            "factoryLogFilters.id"
          ).select("factoryId").where("chainId", "=", chainId).where("startBlock", "<", encodeAsText(block)).where("endBlock", ">", encodeAsText(block))
        ).updateTable("factoryLogFilterIntervals").set({
          endBlock: encodeAsText(block)
        }).where(
          "factoryId",
          "in",
          sql8`(SELECT "factoryId" FROM ${sql8.table("updateFactoryLogFilter")})`
        ).execute();
        await tx.with(
          "deleteTraceFilter(traceFilterId)",
          (qb) => qb.selectFrom("traceFilterIntervals").innerJoin("traceFilters", "traceFilterId", "traceFilters.id").select("traceFilterId").where("chainId", "=", chainId).where("startBlock", ">=", encodeAsText(block))
        ).deleteFrom("traceFilterIntervals").where(
          "traceFilterId",
          "in",
          sql8`(SELECT "traceFilterId" FROM ${sql8.table("deleteTraceFilter")})`
        ).execute();
        await tx.with(
          "updateTraceFilter(traceFilterId)",
          (qb) => qb.selectFrom("traceFilterIntervals").innerJoin("traceFilters", "traceFilterId", "traceFilters.id").select("traceFilterId").where("chainId", "=", chainId).where("startBlock", "<", encodeAsText(block)).where("endBlock", ">", encodeAsText(block))
        ).updateTable("traceFilterIntervals").set({
          endBlock: encodeAsText(block)
        }).where(
          "traceFilterId",
          "in",
          sql8`(SELECT "traceFilterId" FROM ${sql8.table("updateTraceFilter")})`
        ).execute();
        await tx.with(
          "deleteFactoryTraceFilter(factoryId)",
          (qb) => qb.selectFrom("factoryTraceFilterIntervals").innerJoin(
          ...
          "factoryId",
          "in",
          sql8`(SELECT "factoryId" FROM ${sql8.table("deleteFactoryTraceFilter")})`
        ).execute();
        await tx.with(
          "updateFactoryTraceFilter(factoryId)",
          (qb) => qb.selectFrom("factoryTraceFilterIntervals").innerJoin(
          ...
          "factoryId",
          "in",
          sql8`(SELECT "factoryId" FROM ${sql8.table("updateFactoryTraceFilter")})`
        ).execute();
        await tx.with(
          "deleteBlockFilter(blockFilterId)",
          (qb) => qb.selectFrom("blockFilterIntervals").innerJoin("blockFilters", "blockFilterId", "blockFilters.id").select("blockFilterId").where("chainId", "=", chainId).where("startBlock", ">=", encodeAsText(block))
        ).deleteFrom("blockFilterIntervals").where(
          "blockFilterId",
          "in",
          sql8`(SELECT "blockFilterId" FROM ${sql8.table("deleteBlockFilter")})`
        ).execute();
        await tx.with(
          "updateBlockFilter(blockFilterId)",
          (qb) => qb.selectFrom("blockFilterIntervals").innerJoin("blockFilters", "blockFilterId", "blockFilters.id").select("blockFilterId").where("chainId", "=", chainId).where("startBlock", "<", encodeAsText(block)).where("endBlock", ">", encodeAsText(block))
        ).updateTable("blockFilterIntervals").set({
          endBlock: encodeAsText(block)
        }).where(
          "blockFilterId",
          "in",
          sql8`(SELECT "blockFilterId" FROM ${sql8.table("updateBlockFilter")})`
        ).execute();
        await tx.deleteFrom("logs").where("chainId", "=", chainId).where("blockNumber", ">=", encodeAsText(block)).execute();
        await tx.deleteFrom("blocks").where("chainId", "=", chainId).where("number", ">=", encodeAsText(block)).execute();
        await tx.deleteFrom("rpcRequestResults").where("chainId", "=", chainId).where("blockNumber", ">=", encodeAsText(block)).execute();
        await tx.deleteFrom("callTraces").where("chainId", "=", chainId).where("blockNumber", ">=", encodeAsText(block)).execute();
        await tx.deleteFrom("transactions").where("chainId", "=", chainId).where("blockNumber", ">=", encodeAsText(block)).execute();
        await tx.deleteFrom("transactionReceipts").where("chainId", "=", chainId).where("blockNumber", ">=", encodeAsText(block)).execute();
      })
    );
  }
};
function buildFactoryChildAddressSelectExpression2({
  ...
  if (childAddressLocation.startsWith("offset")) {
    ...
    const start4 = 2 + 12 * 2 + 1;
    const length = 20 * 2;
    return sql8`'0x' || substring(${sql8.ref(childAddressLocation)}, ${start4}, ${length})`;
  }
}
...
function createQueue2({
  ...
  queue.addTask = async (task, taskOptions) => {
    ...
    try {
      ...
      await onError2?.({ error: error_, task, queue });
    }
  };
  return queue;
}
...
import { hexToNumber as hexToNumber5, numberToHex, toHex } from "viem";
...
var HistoricalSyncService = class extends Emittery {
  ...
  factoryChildAddressTaskWorker = async ({
    ...
    const { isUpdated, prevCheckpoint, newCheckpoint } = this.factoryChildAddressProgressTrackers[factory.id].addCompletedInterval([fromBlock, toBlock]);
    switch (factory.type) {
      case "factoryLog": {
        if (isUpdated) {
...
var isInBloom = (_bloom, input) => {
  const bloom = hexToBytes2(_bloom);
  const hash2 = hexToBytes2(keccak256(input));
...
var handleBlock = async (service, { newHeadBlock }) => {
  ...
  if (shouldRequestLogs === false && (service.logSources.length > 0 || service.factoryLogSources.length > 0)) {
    service.common.logger.debug({
      service: "realtime",
...
var getMostRecentBlock = (service, checkpoint) => {
...
var getMatchedLogs = async (service, {
  ...
  if (service.factoryLogSources.length === 0) {
    ...
    const matchedFactoryLogs = filterLogs({
      logs,
      logFilters: service.factoryLogSources.map((fs2) => ({
        address: fs2.criteria.address,
        topics: [fs2.criteria.eventSelector]
      }))
    });
        ...
        if (request.method === "eth_getLogs" && isHex2(request.params[0].fromBlock) && isHex2(request.params[0].toBlock)) {
          ...
          common.logger.debug({
            ...
            msg: `Caught eth_getLogs error on '${network.name}', retrying with ranges: [${getLogsErrorResponse.ranges.map(
              ({ fromBlock, toBlock }) => `[${hexToBigInt4(fromBlock).toString()}, ${hexToBigInt4(toBlock).toString()}]`
            ).join(", ")}].`
          });
          const logs = [];
...
var create4 = async ({
  ...
  const onRealtimeSyncEvent = (realtimeSyncEvent) => {
    switch (realtimeSyncEvent.type) {
      case "checkpoint": {
        ...
        if (networkService.realtime === void 0)
          return;
      ...
      case "finalize": {
        ...
        if (networkService.realtime.endBlock !== void 0 && realtimeSyncEvent.checkpoint.blockNumber > networkService.realtime.endBlock) {
          common.logger.info({
            service: "sync",
          ...
          networkService.realtime = void 0;
        }
  ...
  const networkServices = await Promise.all(
    networks.map(async (network) => {
      ...
      const [
        ...
        hasEndBlock ? _eth_getBlockByNumber(
          { requestQueue },
          {
      ...
      for (const source of networkSources) {
        if (source.startBlock > hexToNumber8(latestBlock.number)) {
          common.logger.warn({
            service: "sync",
            msg: `Start block ${source.startBlock} is greater than the latest block ${hexToNumber8(
              latestBlock.number
            )} for '${network.name}'.`
          });
        }
      ...
      const startCheckpoint2 = {
        ...zeroCheckpoint,
        blockTimestamp: hexToNumber8(startBlock.timestamp),
        blockNumber: hexToBigInt6(startBlock.number),
        chainId: BigInt(network.chainId)
      };
      const endCheckpoint = endBlock ? {
        ...zeroCheckpoint,
        blockTimestamp: hexToNumber8(endBlock.timestamp),
        blockNumber: hexToBigInt6(endBlock.number),
        chainId: BigInt(network.chainId)
      } : void 0;
      ...
      if (canSkipRealtime) {
        return {
          ...
          startCheckpoint: startCheckpoint2,
          endCheckpoint,
        ...
        return {
          ...
          realtime: {
            ...
            endBlock: networkSources.every(
              (source) => source.endBlock !== void 0
            ) ? Math.max(...networkSources.map((source) => source.endBlock)) : void 0
          },
          historical: {
            historicalSync,
            checkpoint: void 0,
  ...
  const syncService = {
    ...
    isKilled: false,
    startCheckpoint,
    endCheckpoint: networkServices.every((ns) => ns.endCheckpoint !== void 0) ? checkpointMax(...networkServices.map((ns) => ns.endCheckpoint)) : void 0,
...
var getStatusBlocks = (syncService, realtimeCheckpoint) => {
  const status = {};
  for (const networkService of syncService.networkServices) {
    if (networkService.realtime === void 0) {
      ...
      const mostRecentBlock = networkService.realtime.realtimeSync.getMostRecentBlock(
        realtimeCheckpoint === void 0 ? syncService.checkpoint : checkpointMin(syncService.checkpoint, realtimeCheckpoint)
      );
      if (mostRecentBlock === void 0) {
        status[networkService.network.name] = void 0;
      } else {
        status[networkService.network.name] = {
...
async function run({
  ...
  const handleEvents = async (events, toCheckpoint) => {
    ...
    worker: async (event) => {
      switch (event.type) {
        case "newEvents": {
          ...
          for (const network of networks) {
            if (statusBlocks[network.name] !== void 0) {
  ...
  let indexingStore = historicalStore;
...
// src/drizzle/hex.ts
import { entityKind as entityKind2 } from "drizzle-orm";
import {
  PgColumn,
  PgColumnBuilder
} from "drizzle-orm/pg-core";
import {
  SQLiteColumn as SQLiteColumn2,
  SQLiteColumnBuilder as SQLiteColumnBuilder2
} from "drizzle-orm/sqlite-core";
import { bytesToHex as bytesToHex2, hexToBytes as hexToBytes3 } from "viem";
var PgHexBuilder = class extends PgColumnBuilder {
...
var PgHex = class extends PgColumn {
  static [entityKind2] = "PgHex";
  getSQLType() {
    return "bytea";
  }
  mapFromDriverValue(value) {
    return bytesToHex2(value);
  }
  mapToDriverValue(value) {
    return Buffer.from(hexToBytes3(value));
  }
};
var SQLiteHexBuilder = class extends SQLiteColumnBuilder2 {
  static [entityKind2] = "SQliteHexBuilder";
  constructor(columnName) {
...
var SQLiteHex = class extends SQLiteColumn2 {
  static [entityKind2] = "SQLiteHex";
  getSQLType() {
    return "blob";
  }
  mapFromDriverValue(value) {
...
// src/drizzle/json.ts
import { entityKind as entityKind3 } from "drizzle-orm";
import {
  SQLiteColumn as SQLiteColumn3,
  SQLiteColumnBuilder as SQLiteColumnBuilder3
} from "drizzle-orm/sqlite-core";
var SQLiteJsonBuilder = class extends SQLiteColumnBuilder3 {
  static [entityKind3] = "SQliteJsonBuilder";
  constructor(columnName) {
...
var SQLiteJson = class extends SQLiteColumn3 {
  static [entityKind3] = "SQLiteJson";
  getSQLType() {
    return "jsonb";
  }
  mapFromDriverValue(value) {
...
// src/drizzle/list.ts
import { entityKind as entityKind4 } from "drizzle-orm";
import {
  PgColumn as PgColumn2,
  PgColumnBuilder as PgColumnBuilder2
} from "drizzle-orm/pg-core";
import {
  SQLiteColumn as SQLiteColumn4,
  SQLiteColumnBuilder as SQLiteColumnBuilder4
} from "drizzle-orm/sqlite-core";
var PgListBuilder = class extends PgColumnBuilder2 {
  static [entityKind4] = "PgListBuilder";
  element;
  constructor(columnName, element) {
...
var PgList = class extends PgColumn2 {
  static [entityKind4] = "PgList";
  element;
  constructor(table, config, element) {
    super(table, config);
    this.element = element;
  }
  getSQLType() {
  ...
  mapFromDriverValue(value) {
    return this.element === "bigint" ? JSON.parse(value).map(BigInt) : JSON.parse(value);
  }
  mapToDriverValue(value) {
    return this.element === "bigint" ? JSON.stringify(value.map(String)) : JSON.stringify(value);
  }
};
var SQLiteListBuilder = class extends SQLiteColumnBuilder4 {
  static [entityKind4] = "SQliteListBuilder";
  element;
  constructor(columnName, element) {
    super(columnName, "string", "PgList");
    this.element = element;
...
var SQLiteList = class extends SQLiteColumn4 {
  static [entityKind4] = "SQLiteList";
  element;
  constructor(table, config, element) {
    super(table, config);
    this.element = element;
...
// src/drizzle/runtime.ts
var createDrizzleDb = (database) => {
  if (database.kind === "postgres") {
    const drizzle = drizzlePg(database.readonlyPool);
    return {
      // @ts-ignore
      select: (...args) => drizzle.select(...args),
      execute: (query2) => drizzle.execute(query2)
    };
  } else {
    ...
    return {
      ...
      execute: (query2) => {
        try {
          try {
            return drizzle.all(query2);
          } catch (e) {
            const error = e;
            if (error.name === "SqliteError" && error.message === "This statement does not return data. Use run() instead") {
              return drizzle.run(query2);
            } else {
              throw error;
            }
          }
...
var createDrizzleTables = (schema, database, dbNamespace) => {
  const drizzleTables = {};
    ...
    for (const [columnName, column] of Object.entries(table)) {
      if (isMaterialColumn(column)) {
        if (isJSONColumn(column)) {
          ...
          if (columnName === "id") {
            drizzleColumns[columnName] = drizzleColumns[columnName].primaryKey();
...
var convertStringColumn = (columnName, kind) => {
  return kind === "sqlite" ? SQLiteText(columnName) : PgText(columnName);
};
var convertIntColumn = (columnName, kind) => {
  return kind === "sqlite" ? SQLiteInteger(columnName) : PgInteger(columnName);
};
var convertFloatColumn = (columnName, kind) => {
  return kind === "sqlite" ? SQLiteReal(columnName) : PgDoublePrecision(columnName);
};
var convertBooleanColumn = (columnName, kind) => {
  return kind === "sqlite" ? SQLiteInteger(columnName) : PgInteger(columnName);
};
var convertHexColumn = (columnName, kind) => {
  return kind === "sqlite" ? new SQLiteHexBuilder(columnName) : new PgHexBuilder(columnName);
};
var convertBigintColumn = (columnName, kind) => {
  return kind === "sqlite" ? new SQLiteBigintBuilder(columnName) : PgNumeric(columnName, { precision: 78 });
};
var convertListColumn = (columnName, kind, element) => {
  return kind === "sqlite" ? new SQLiteListBuilder(columnName, element) : new PgListBuilder(columnName, element);
};
var convertJsonColumn = (columnName, kind) => {
  return kind === "sqlite" ? new SQLiteJsonBuilder(columnName) : PgJsonb(columnName);
};
var convertEnumColumn = (columnName, kind) => {
  return kind === "sqlite" ? SQLiteText(columnName) : PgText(columnName);
};

// src/hono/index.ts
var applyHonoRoutes = (hono, routes, customContext) => {
  const addCustomContext = (handler) => (c, next) => {
  ...
  for (const {
    ...
    if (method === "GET" || method === "POST") {
      if (typeof maybePathOrHandler === "string") {
        path11 = maybePathOrHandler;
      } else {
        hono.addRoute(method, path11, addCustomContext(maybePathOrHandler));
      }
      for (const handler of handlers) {
        if (typeof handler !== "string") {
          hono.addRoute(method, path11, addCustomContext(handler));
        }
      }
    } else {
      if (typeof maybePathOrHandler === "string") {
      ...
      for (const handler of handlers) {
        hono.addRoute("ALL", path11, addCustomContext(handler));
      }
    }
  }
  return hono;
...
var onError = async (_error, c, common) => {
  ...
  const errorFile = (() => {
    if (!matches?.[0])
      return void 0;
    const path11 = matches[0].trim();
    if (path11.startsWith("(")) {
      return path11.slice(1);
    } else if (path11.startsWith("file://")) {
      return path11.slice(7);
    }
    return path11;
  })();
  addStackTrace(error, common.options);
...
async function createServer2({
  ...
  if (userRoutes.length === 0 && userApp.routes.length === 0) {
    ...
    applyHonoRoutes(hono, userRoutes, { db, tables }).onError(
...
async function runServer({
  ...
  const server = await createServer2({
    ...
    schema,
    database,
    dbNamespace: databaseConfig.kind === "sqlite" ? "public" : databaseConfig.schema
  });
  return async () => {
    await server.kill();
  };
...
var devCommand = new Command("dev").description("Start the development server with hot reloading").option("-p, --port <PORT>", "Port for the web server", Number, 42069).option("-H, --hostname <HOSTNAME>", "Hostname for the web server", "0.0.0.0").showHelpAfterError().action(async (_, command) => {
  const cliOptions = {
...
var startCommand = new Command("start").description("Start the production server").option("-p, --port <PORT>", "Port for the web server", Number, 42069).option("-H, --hostname <HOSTNAME>", "Hostname for the web server", "0.0.0.0").showHelpAfterError().action(async (_, command) => {
  const cliOptions = {
    ...command.optsWithGlobals(),
    command: command.name()
  };
...
var serveCommand = new Command("serve").description("Start the production HTTP server without the indexer").option("-p, --port <PORT>", "Port for the web server", Number, 42069).option("-H, --hostname <HOSTNAME>", "Hostname for the web server", "0.0.0.0").showHelpAfterError().action(async (_, command) => {
  const cliOptions = {
    ...command.optsWithGlobals(),
    command: command.name()
  };
